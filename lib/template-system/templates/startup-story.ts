...
export const startupStoryTemplate: BuiltInTemplate = {
    id: "startup-story-v1",
    schema: {
        id: "startup-story-v1",
    ...
    generateCode: (values) => {
        const title = sanitizeString(values.title, "WE JUST HIT");
        const metric = sanitizeString(values.metric, "$100k MRR");
        const subtitle = sanitizeString(values.subtitle, "After 12 months of building entirely in public");
