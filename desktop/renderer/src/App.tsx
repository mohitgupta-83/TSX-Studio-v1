        ...
        <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-blue-500/30 flex overflow-hidden">
            ...
            <aside className="w-64 border-r border-white/5 bg-black/40 backdrop-blur-xl flex flex-col p-6 space-y-8 z-50">
                ...
                <nav className="flex-1 space-y-2">
                    ...
                    <NavItem icon={<Mic className="w-4 h-4" />} label="Transcribe" active={view === 'transcribe'} onClick={() => setView('transcribe')} />
            ...
            <main className="flex-1 relative overflow-auto bg-[radial-gradient(circle_at_50%_0%,rgba(59,130,246,0.05),transparent)]">
                {view === 'dashboard' ? (
                    <div className="p-10 max-w-6xl mx-auto space-y-12">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                            ...
                            <div className="flex items-center gap-4">
                                {/* Credits Component - Matches High-End Screenshot */}
                        ...
                        <div className="flex items-center gap-4">
                            <div className="relative flex-1 group">
                                <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground transition-colors group-focus-within:text-blue-500" />
                                <input 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-12">
                            {projects.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase())).map(project => (
                                <ProjectCard key={project.id} project={project} onDelete={(id, e) => deleteProject(id, e)} onClick={() => setView('studio')} />
                            ))}
                    ...
                    <div className="absolute inset-0 z-[100] p-12 flex flex-col items-center justify-center text-center space-y-8 bg-[#050505]">
                         <div className="w-24 h-24 bg-blue-500/10 rounded-[32px] flex items-center justify-center border border-blue-500/20 shadow-[0_0_50px_rgba(59,130,246,0.1)]">
                            <MonitorPlay className="w-10 h-10 text-blue-500" />
                        </div>
                        <div className="max-w-md space-y-4">
                            <h2 className="text-4xl font-black italic uppercase tracking-tighter leading-none">Standalone <span className="text-blue-500">Engine</span></h2>
                            <p className="text-sm text-muted-foreground leading-relaxed font-medium">
                                To ensure maximum output performance, the Visual Editor requires the local studio environment. 
                                <span className="text-white font-bold ml-1">Connect to local developer server</span> to access.
                            </p>
                        </div>
                        <div className="flex gap-4">
                            <button 
                                onClick={() => window.open('http://localhost:3000', '_blank')}
                                className="h-14 px-12 bg-white text-black text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-blue-500 hover:text-white transition-all shadow-xl shadow-white/5"
                            >
                                Open Visual Editor <ExternalLink className="w-4 h-4 ml-3 inline" />
                            </button>
                            <button 
                                onClick={() => setView('dashboard')}
                                className="h-14 px-10 bg-transparent border border-white/10 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-white/5 transition-all"
                            >
                                Back to Dashboard
                            </button>
                        </div>
                        <div className="pt-12 flex items-center gap-6 opacity-30 grayscale saturate-0 pointer-events-none">
                            <div className="h-[1px] w-12 bg-white/20" />
                            <p className="text-[10px] font-black uppercase tracking-widest">or run <span className="text-white">npm run dev</span> in root</p>
                            <div className="h-[1px] w-12 bg-white/20" />
                        </div>
                    </div>
                )}
...
const NavItem = ({ icon, label, active = false, onClick, disabled = false }: { icon: any, label: string, active?: boolean, onClick?: () => void, disabled?: boolean }) => (
    <button 
        onClick={onClick}
        disabled={disabled}
...
const ProjectCard = ({ project, onDelete, onClick }: { project: Project, onDelete: (id: string, e: React.MouseEvent) => void, onClick: () => void }) => {
    return (
        <div 
            onClick={onClick}
